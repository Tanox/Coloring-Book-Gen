/* app/components/ResultsGallery.tsx v0.2.6 */
  ...
  return (
    <div className="animate-fade-in-up">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-12 gap-6">
            <h3 className="text-4xl sm:text-5xl font-extrabold text-slate-800 dark:text-white drop-shadow-none">{t('resultsTitle')}</h3>
            <button 
                onClick={onDownload}
                disabled={isDownloading}
                className="flex items-center gap-3 bg-green-500 hover:bg-green-600 text-white px-10 py-4 rounded-2xl text-xl font-bold transition-all disabled:opacity-70 disabled:cursor-wait disabled:transform-none active:translate-y-[2px]"
            >
                {isDownloading ? (
                        <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {pages.map((page, idx) => (
                <div key={idx} className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border-2 border-slate-100 dark:border-slate-700 group hover:scale-[1.02] hover:rotate-1 transition-all duration-300 flex flex-col relative overflow-hidden shadow-none">
                    ...
                    <div className="aspect-[3/4] w-full overflow-hidden rounded-2xl bg-slate-50 dark:bg-slate-900 mb-5 border-2 border-dashed border-slate-200 dark:border-slate-700 relative group-hover:border-indigo-300 transition-colors">
                        ...
                        <div className="absolute inset-0 bg-indigo-900/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-4">
                            <button 
                                ...
                                className="bg-white text-indigo-600 p-4 rounded-full hover:scale-110 hover:bg-indigo-50 transition-all border-2 border-indigo-100 shadow-none"
                                title={t('btnColorNow')}
                            >
                            ...
                            <button 
                                ...
                                className="bg-white text-pink-600 p-4 rounded-full hover:scale-110 hover:bg-pink-50 transition-all border-2 border-pink-100 shadow-none"
